...
func serversCreateCmd() *cobra.Command {
	...
	c.Flags().StringVar(&namespace, "namespace", defaults.namespace, "namespace to create/edit the per-server config override ConfigMap in (kubectl-only, not sent to gateway; env MAGPIE_NAMESPACE, or 'magpiectl target')")
