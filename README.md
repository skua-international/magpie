...
- **`gateway`** — the cluster's public entrypoint, and JWT-authenticated `ArmaServer` CRUD plus deployment-like lifecycle (`Create`/`Start`/`Stop`/`Update`). Also serves the web UI's static assets under `/ui` (outside the auth middleware — the bundle has to load before anyone can sign in through it) and reads pod status/logs for `GetServerHealth`/`GetServerLogs`. Deliberately carries the least Kubernetes RBAC of anything here — `ArmaServer` read/write plus **read-only** `pods`/`pods/log`, still no `Deployment` access at all — because it's the one process a stolen bearer token can reach directly; logs sit behind their own `servers:logs` scope so listing servers doesn't imply reading everything they printed. Named `server-api` until it took the `/` catch-all on the single public host and stopped being only the `ArmaServer` API.
- **`registry`** — JWT-authenticated mod source registry (a Steam mod, a collection, a preset export, or a locally-uploaded zip mod) and mission (`.pbo`) storage, backed directly by the `ModSource` CRD. Also the one place that can export/import declarative cluster state (`admin export-state`/`import-state`, `admin:export`/`admin:import` scopes) — mod source registrations, ConfigMaps, and `ArmaServer` specs, deliberately excluding synced file content, Postgres data, ACL grants, and any live credential — which is why it's also the only service besides `controller`/`gateway` with any `ArmaServer`/`ConfigMap` RBAC at all.
...
1. Register an OAuth2 app with the provider(s) you want (Discord Developer Portal / GitHub OAuth Apps / Google Cloud Console), with its redirect URI set to `<identity.baseUrl>/auth/<provider>/callback` (e.g. `https://api.example.com/auth/discord/callback`). `identity.baseUrl` defaults to the cluster's single public host — `http[s]://<ingress.host>.<ingress.baseDomain>` — so you normally don't set it yourself; it only needs an explicit value when something other than this chart fronts the cluster. Whatever it resolves to has to match what's registered here byte for byte, and has to be reachable from wherever the browser completing a login is.
   Where a login is allowed to *return* to is a separate control: identity accepts a `redirect_uri` only if it's loopback (that's `magpiectl`'s local callback listener, on an ephemeral port) or its origin matches `identity.baseUrl`'s. Since anything this chart serves publicly is same-origin with `baseUrl`, that covers the normal cases with no configuration. A UI hosted on some *other* origin needs it listed in `identity.allowedRedirectOrigins`, which is an open-redirect guard — only add origins you control.
...
The cluster exposes a **single public host**, `<ingress.host>.<ingress.baseDomain>` (default `api.magpie.local`), which routes to `gateway`, `registry` and `identity` by path prefix. Set it once with `magpiectl target` and later invocations pick it up automatically; `MAGPIE_API_URL` and `--api-url` override it in that order.
